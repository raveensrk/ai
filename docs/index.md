# Docs Index

Index of documentation in this directory.

- [Agent Notes](./agents_notes.md) — Insights and working practices for AI agents.
- [AI Agents](./ai_agents.md) — Notes on working effectively with AI coding agents.
- [Bookmarks](./bookmarks.md) — Curated resources and reading list.
- [Claude Starter Pack — 2026 Edition](./claude_starter_pack_2026.md) — Practical user guide to building with Claude.
- [Claude Notes](./claude.md) — Personal notes and rules for working with Claude in this project.
- [Codex Starter Pack 2026](./codex_starter_pack_2026.md) — Practical user guide to building with Codex.
- [Common Rules](./common.md) — Shared rules for agents working in this repository.
- [Emoji Legend](./emoji_legend.md) — Canonical emoji vocabulary for agent reports and summaries.
- [Inbox](./inbox.md) — Incoming tasks and ideas.
- [Maintenance](./maintenance.md) — Repository maintenance workflow.
- [Prompts](./prompts.md) — Reusable prompts organized by workflow stage.
- [Tasks](./tasks.md) — Tasks that AI agents can perform.
- [Terminologies](./terminologies.md) — Shared prompting vocabulary.
- [Todo Schema](./todo_schema.md) — Canonical format for todo items in documentation files.
- [Use Case](./use_case.md) — Repository use cases.
- [Write Docs](./write_docs.md) — Documentation-writing workflow.
